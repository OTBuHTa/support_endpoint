...
cleanup() {
  docker compose -f "$compose_file" exec -T postgres \
    sh -ceu 'dropdb --if-exists --force "$1"' sh "$verify_db" >/dev/null 2>&1 || true
}
trap cleanup EXIT

docker compose -f "$compose_file" exec -T postgres \
  sh -ceu 'createdb "$1"' sh "$verify_db"

docker compose -f "$compose_file" exec -T postgres \
  sh -ceu 'pg_restore --exit-on-error --no-owner --no-acl --dbname="$1"' sh "$verify_db" \
  < "$backup_file"

docker compose -f "$compose_file" exec -T postgres \
  sh -ceu 'psql --dbname="$1" --tuples-only --no-align --command="SELECT count(*) FROM alembic_version;"' \
  sh "$verify_db" \
  | grep -Eq '^[1-9][0-9]*$'
